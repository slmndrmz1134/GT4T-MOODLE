...
if (mutenancy_is_active()) {
    ...
    login_signup_prepare_tenant_context();
}

if (!$authplugin = login_resolve_signup_authplugin()) {
    // Do not throw here: Moodle's exception handler sends HTTP 404 and Chrome logs
    // "signup.php Failed to load resource (404)" even though HTML is shown.
    $loginurl = new moodle_url('/login/index.php');
    if (mutenancy_is_active()) {
        $current = (int) \tool_mutenancy\local\tenancy::get_current_tenantid();
        if ($current) {
            $tenant = \tool_mutenancy\local\tenant::fetch($current);
            if ($tenant && !$tenant->archived) {
                $loginurl->param('tenant', $tenant->idnumber);
            }
        }
        redirect($loginurl, get_string('signupsitenotavailable', 'theme_moove'), null,
            \core\output\notification::NOTIFY_ERROR);
    }
    redirect($loginurl, get_string('notlocalisederrormessage', 'error',
        'Sorry, you may not use this page.'), null, \core\output\notification::NOTIFY_ERROR);
}
...
if ($mform_signup->is_cancelled()) {
    ...
    if (mutenancy_is_active() && !empty($user->tenantid)) {
        \tool_mutenancy\local\tenancy::switch((int) $user->tenantid);
    }
...
// Successful signup form must not inherit a 404 from earlier handlers.
if (!headers_sent()) {
