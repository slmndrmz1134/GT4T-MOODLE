...
/**
 * Whether self-registration is enabled for a tenant.
 *
 * @param int $tenantid
 * @return bool
 */
function login_tenant_signup_enabled(int $tenantid): bool {
    if (!mutenancy_is_active()) {
        return false;
    }

    $registerauth = \tool_mutenancy\local\config::get($tenantid, 'core', 'registerauth');
    ...
    return get_auth_plugin($registerauth)->can_signup();
}

/**
 * Tenants available on the signup form (id => label).
 *
 * @return array<int|string, string>
 */
function login_get_signup_tenant_choices(): array {
    global $DB;
    ...
    // Signup list: all active tenants with self-registration (loginshow is only for login picker).
    $tenants = $DB->get_records_sql(
        "SELECT t.id, t.name, t.idnumber, t.sitefullname
    ...
    $choices = [];
    if (count($tenants) > 1) {
        $choices[''] = get_string('signuptenantchoose', 'theme_moove');
    }

    foreach ($tenants as $tenant) {
        if (!login_tenant_signup_enabled((int) $tenant->id)) {
            continue;
        }
        $label = $tenant->sitefullname ?: $tenant->name;
        $choices[(int) $tenant->id] = format_string($label);
    }
...
/**
 * Validate tenant selected on signup.
 *
 * @param int $tenantid
 * @return bool
 */
function login_is_valid_signup_tenant(int $tenantid): bool {
    global $DB;

    if (!mutenancy_is_active() || $tenantid <= 0) {
        return false;
    }

    if (!login_tenant_signup_enabled($tenantid)) {
        return false;
    }

    return $DB->record_exists('tool_mutenancy_tenant', [
        'id' => $tenantid,
        'archived' => 0,
    ]);
}
...
/**
 * Tenant ids that allow self-registration.
 *
 * @return int[]
 */
function login_get_signup_tenant_ids(): array {
    $choices = login_get_signup_tenant_choices();

    return array_values(array_filter(array_keys($choices), static function($key): bool {
        return $key !== '';
    }));
}

/**
 * Is self-registration enabled at site level (Administration → Plugins → Auth).
 *
 * @return bool
 */
function login_site_signup_enabled(): bool {
    $registerauth = get_config('core', 'registerauth');
    if ($registerauth === false || $registerauth === '' || $registerauth === null) {
        return false;
    }
...
/**
 * Is self-registration available for at least one university?
 *
 * @return bool
 */
function login_any_signup_available(): bool {
    if (!mutenancy_is_active()) {
        return login_site_signup_enabled();
    }

    if (count(login_get_signup_tenant_ids()) > 0) {
        return true;
    }

    // Signup page may use site default when no tenant override allows registration.
    return login_site_signup_enabled();
}

/**
 * Should the login page show the "Create new account" button?
 *
 * Uses the same rules as the signup page so the button matches actual availability.
 *
 * @return bool
 */
function login_should_show_signup_button(): bool {
    if (isloggedin() && !isguestuser()) {
        return false;
    }

    return login_any_signup_available();
}

/**
 * Canonical signup page URL (preserves tenant when that university allows signup).
 *
 * @return \moodle_url
 */
function login_get_signup_url(): moodle_url {
    $url = new moodle_url('/login/signup.php');

    if (mutenancy_is_active()) {
        $tenantid = (int) \tool_mutenancy\local\tenancy::get_current_tenantid();
        if ($tenantid && login_tenant_signup_enabled($tenantid)) {
            $tenant = \tool_mutenancy\local\tenant::fetch($tenantid);
            if ($tenant && !$tenant->archived) {
                $url->param('tenant', $tenant->idnumber);
            }
        }
    }
...
/**
 * Resolve auth plugin for the signup page (multi-tenant aware).
 *
 * When the active tenant (from ?tenant=) does not allow signup, falls back to any
 * tenant that does — the form still requires the user to pick a university.
 *
 * @return auth_plugin_base|false
 */
function login_resolve_signup_authplugin() {
    if (!mutenancy_is_active()) {
        return signup_is_enabled();
    }

    $tenantids = login_get_signup_tenant_ids();
    if ($tenantids === []) {
        if (!login_site_signup_enabled()) {
            return false;
        }
        $registerauth = get_config('core', 'registerauth');
        $authplugin = get_auth_plugin($registerauth);

        return $authplugin->can_signup() ? $authplugin : false;
    }

    $current = (int) \tool_mutenancy\local\tenancy::get_current_tenantid();
    $tenantid = ($current && login_tenant_signup_enabled($current)) ? $current : $tenantids[0];

    $registerauth = \tool_mutenancy\local\config::get($tenantid, 'core', 'registerauth');
    if (empty($registerauth) || !is_enabled_auth($registerauth)) {
        return false;
    }
...
/**
 * If ?tenant= points to a university without self-registration, clear tenant context.
 *
 * @return void
 */
function login_signup_prepare_tenant_context(): void {
    if (!mutenancy_is_active()) {
        return;
    }

    if (login_get_signup_tenant_ids() === []) {
        return;
    }

    $current = (int) \tool_mutenancy\local\tenancy::get_current_tenantid();
    if ($current && !login_tenant_signup_enabled($current)) {
        \tool_mutenancy\local\tenancy::switch(0);
        \tool_mutenancy\local\tenancy::fix_site(0);
    }
}
