...
use tool_mutenancy\external\remove_manager;
 ...
 *
 * @covers \tool_mutenancy\external\remove_manager
 */
final class remove_manager_test extends \advanced_testcase {
    ...
    public function test_definition(): void {
        $function = \core_external\external_api::external_function_info('tool_mutenancy_remove_manager');
        $this->assertSame(remove_manager::class, $function->classname);
    ...
    public function test_execute(): void {
        ...
        $this->assertTrue(remove_manager::execute($tenant1->id, $user1->id));
        $this->assertFalse($DB->record_exists('tool_mutenancy_manager', ['tenantid' => $tenant1->id, 'userid' => $user1->id]));

        $this->assertFalse(remove_manager::execute($tenant1->id, $user1->id));
        $this->assertFalse($DB->record_exists('tool_mutenancy_manager', ['tenantid' => $tenant1->id, 'userid' => $user1->id]));

        $this->setUser($user);
        try {
            remove_manager::execute($tenant1->id, $user1->id);
