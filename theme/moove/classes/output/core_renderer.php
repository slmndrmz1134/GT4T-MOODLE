...
class core_renderer extends \theme_boost\output\core_renderer {
    ...
    public function standard_head_html() {
        ...
        // GT4T landing + login (Montserrat, fixed mobile viewport).
        global $PAGE;
        if (in_array($PAGE->pagelayout, ['frontpage', 'login'], true)) {
            $viewport = '<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, '
                . 'minimum-scale=1, user-scalable=no, viewport-fit=cover">';
            $output = preg_replace('/<meta name="viewport" content="[^"]*">/', $viewport, $output, 1);

            // Subset weights only (smaller CSS, faster LCP than loading 300–800).
            $output .= '<link rel="preconnect" href="https://fonts.googleapis.com">
                        <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    ...
    public function should_display_logo() {
        ...
        // Hero section shows the GT4T logo centred; avoid a second logo in the navbar.
     ...
     *
     * Tenant manager logos (core_admin / compact) take priority over the Moove GT4T default.
     *
     * @return string|false
     */
    public function get_logo() {
        ...
        // Front page and login pages always use the GT4T logo; tenant logos apply on other pages.
        if ($PAGE->pagelayout === 'frontpage' || $PAGE->pagelayout === 'login') {
            return \theme_moove_get_gt4t_logo_url();
        }
        ...
        if (function_exists('mutenancy_is_active') && mutenancy_is_active()) {
            $boostlogo = \tool_mutenancy\local\appearance::get_boost_setting_image_url('logo');
            if ($boostlogo) {
                return $boostlogo->out(false);
            }
        }

        $themelogo = $this->get_theme_logo_url();
        if ($themelogo) {
            return $themelogo;
        }
    ...
    public function render_login(\core_auth\output\login $form) {
        ...
        if (function_exists('mutenancy_is_active') && mutenancy_is_active()
                && class_exists('\\tool_mutenancy\\output\\logintenantselector')) {
            $tenantselect = new \tool_mutenancy\output\logintenantselector();
            if ($tenantselect->has_items()) {
                $context->tenantselect = $this->render($tenantselect);

                // Get current active tenant details to pass to the template.
                $currenttenantid = (int) \tool_mutenancy\local\tenancy::get_current_tenantid();
                if ($currenttenantid) {
                    $tenant = \tool_mutenancy\local\tenant::fetch($currenttenantid);
                    if ($tenant && !$tenant->archived) {
                        $context->activetenantidnumber = $tenant->idnumber;
                        $context->activetenantname = format_string($tenant->sitefullname ?? $tenant->name);
                    }
                }
            }
        }
        ...
        $context->showsignupbutton = login_should_show_signup_button();
        $context->cansignup = $context->showsignupbutton;
        $context->signupurl = login_get_signup_url()->out(false);
        $context->togglepassword = true;
    ...
    /**
     * Render the login signup form into the GT4T sign-in card layout.
     *
    ...
    public function render_login_signup_form($form) {
        ...
        if (mutenancy_is_active()) {
            ...
            if ($tenantid) {
                ...
                if ($tenant && !$tenant->archived) {
                    $loginurl->param('tenant', $tenant->idnumber);
                }
            }
        }
        $context['loginurl'] = $loginurl->out(false);
        $context['cansignup'] = login_any_signup_available();
        $context['signupurl'] = login_get_signup_url()->out(false);
    ...
    public function favicon() {
        if (function_exists('mutenancy_is_active') && mutenancy_is_active()) {
            $tenantfavicon = \tool_mutenancy\local\appearance::get_favicon_url();
            if ($tenantfavicon) {
                return $tenantfavicon;
            }
        }

        return \theme_moove_get_gt4t_favicon_url();
    }
